...
const LUCI_SECTION_ICON_FALLBACK_OPTIONS = [
  ["port_up", _("Port up"), "/luci-static/resources/icons/port_up.svg"],
  ["port_down", _("Port down"), "/luci-static/resources/icons/port_down.svg"],
  ["switch", _("Switch"), "/luci-static/resources/icons/switch.svg"],
  ["bridge", _("Bridge"), "/luci-static/resources/icons/bridge.svg"],
  ["ethernet", _("Ethernet"), "/luci-static/resources/icons/ethernet.svg"],
  ["wifi", _("Wi-Fi"), "/luci-static/resources/icons/wifi.svg"],
  ["signal-000-000", _("Signal empty"), "/luci-static/resources/icons/signal-000-000.svg"],
  ["signal-025-050", _("Signal low"), "/luci-static/resources/icons/signal-025-050.svg"],
  ["signal-050-075", _("Signal medium"), "/luci-static/resources/icons/signal-050-075.svg"],
  ["signal-075-100", _("Signal high"), "/luci-static/resources/icons/signal-075-100.svg"],
];
let luciSectionIconOptionsPromise = null;
let luciSectionIconOptions = LUCI_SECTION_ICON_FALLBACK_OPTIONS;
const ANNOTATED_TEXTAREA_STYLE_ID = "pdk-annotated-textarea-styles";
const NFQWS_REMOTE_VALIDATION_DEBOUNCE_MS = 500;
const NFQWS_VALIDATION_COMMAND = "/usr/bin/krot";
...
function getSectionIconValue(section_id) {
  return uci.get(UCI_PACKAGE, section_id, "icon") || "port_up";
}

function getSectionIconPath(icon) {
  const normalized = `${icon || "port_up"}`.replace(/\.svg$/, "");
  const found = luciSectionIconOptions.find(([value]) => value === normalized);
  if (found) {
    return found[2];
  }
  const safe = /^[a-zA-Z0-9_-]+$/.test(normalized) ? normalized : "port_up";
  return `/luci-static/resources/icons/${safe}.svg`;
}

function renderSectionIcon(icon) {
  return `<img src="${getSectionIconPath(icon)}" style="width:24px;height:24px;vertical-align:middle" alt="">`;
}

function renderSectionIconNode(icon) {
  return E("img", {
    src: getSectionIconPath(icon),
    alt: "",
    style: "width:24px;height:24px;vertical-align:middle",
  });
}

function getLuciSectionIconOptions() {
  if (luciSectionIconOptionsPromise) {
    return luciSectionIconOptionsPromise;
  }

  luciSectionIconOptionsPromise = fs
    .list("/www/luci-static/resources/icons")
    .then((entries) => {
      const icons = (entries || [])
        .map((entry) => `${entry.name || ""}`)
        .filter((name) => /^[A-Za-z0-9_.-]+\.svg$/.test(name))
        .map((name) => name.replace(/\.svg$/, ""))
        .sort((left, right) => left.localeCompare(right))
        .map((name) => [
          name,
          name,
          `/luci-static/resources/icons/${name}.svg`,
        ]);

      if (icons.length) {
        luciSectionIconOptions = icons;
      }

      return luciSectionIconOptions;
    })
    .catch(() => luciSectionIconOptions);

  return luciSectionIconOptionsPromise;
}

function populateSectionIconOptions(option, icons) {
  delete option.keylist;
  delete option.vallist;

  (icons || luciSectionIconOptions).forEach(([value, label, path]) => {
    option.value(
      value,
      `${label} (${path.replace("/luci-static/resources/icons/", "")})`,
    );
  });
}
...
function cacheNfqwsRemoteValidation(value, result) {
  ...
  if (nfqwsRemoteValidationCache.size > 100) {
    nfqwsRemoteValidationCache.delete(
      nfqwsRemoteValidationCache.keys().next().value,
    );
  }
...
function getNfqwsForbiddenTokenInfo(token, index) {
  const configFileMessage = _(
    "External nfqws config files bypass K.R.O.T. queue management and explicit validation.",
  );
  ...
  const placeholderMessage = _(
    "Zapret hostlist templates are not supported here because K.R.O.T. does not expand them for per-rule NFQWS strategies.",
  );
  const queueMessage = _(
    "The NFQUEUE number is assigned by K.R.O.T. for each rule and must not be overridden here.",
  );
  const fwmarkMessage = _(
    "The desync fwmark is managed by K.R.O.T. for loop prevention and must not be overridden here.",
  );
  const daemonMessage = _(
    "K.R.O.T. manages the nfqws process lifecycle itself, so daemon mode is not allowed here.",
  );
...
function cacheByedpiRemoteValidation(value, result) {
  ...
  if (byedpiRemoteValidationCache.size > 100) {
    byedpiRemoteValidationCache.delete(
      byedpiRemoteValidationCache.keys().next().value,
    );
  }
...
function getByedpiControlledTokenInfo(token) {
  const listenMessage = _(
    "ByeDPI listen address and port are assigned by K.R.O.T. and must not be set in the strategy.",
  );
  const transparentMessage = _(
    "Transparent proxy mode is incompatible with action=byedpi because K.R.O.T. connects to ciadpi through SOCKS.",
  );
  const daemonMessage = _(
    "K.R.O.T. manages the ciadpi process lifecycle itself, so daemon mode is not allowed here.",
  );
  const pidfileMessage = _(
    "K.R.O.T. manages ciadpi pid files itself, so pidfile options are not allowed here.",
  );
  const exitMessage = _(
...
function createSectionContent(section) {
  ...
  o = section.taboption("settings", form.DummyValue, "_icon_display", _("Icon"));
  o.modalonly = false;
  o.rawhtml = true;
  o.width = "4rem";
  o.cfgvalue = function (section_id) {
    return renderSectionIcon(getSectionIconValue(section_id));
  };
  o.textvalue = function (section_id) {
    return getSectionIconValue(section_id);
  };
  o.renderWidget = function (section_id) {
    return E(
      "div",
      { style: "display:flex;align-items:center;justify-content:center" },
      renderSectionIconNode(getSectionIconValue(section_id)),
    );
  };
  ...
  o.write = function (section_id, value) {
    ...
    if (normalized) {
      uci.set(UCI_PACKAGE, section_id, "label", normalized);
    } else {
      uci.unset(UCI_PACKAGE, section_id, "label");
    }
  };
  ...
  o = section.taboption(
    ...
    "icon",
    _("Icon"),
    _("Icon shown in Sections and Dashboard"),
  );
  populateSectionIconOptions(o);
  o.default = "port_up";
  o.rmempty = false;
  o.modalonly = true;
  o.load = function (section_id) {
    return getLuciSectionIconOptions().then((icons) => {
      populateSectionIconOptions(this, icons);
      return getSectionIconValue(section_id);
    });
  };
  o.renderWidget = function (section_id, option_index, cfgvalue) {
    populateSectionIconOptions(this);
    const widget = form.ListValue.prototype.renderWidget.apply(this, [
      section_id,
      option_index,
      cfgvalue,
    ]);
    const preview = E(
      "span",
      {
        class: "pdk-section-icon-preview",
        style:
          "display:inline-flex;align-items:center;justify-content:center;margin-left:.75em;min-width:28px;height:28px;vertical-align:middle",
      },
      renderSectionIconNode(cfgvalue || getSectionIconValue(section_id)),
    );
    const select = widget.querySelector("select");
    const updatePreview = () => {
      preview.replaceChildren(renderSectionIconNode(select?.value || "port_up"));
    };
  ...
  o = section.taboption(
    ...
    "action",
    _("Action"),
    _("What K.R.O.T. should do when this section matches"),
  );
  ...
  o = section.taboption(
    ...
    _("ByeDPI Strategy"),
    _(
      "ciadpi command options. K.R.O.T. manages the listen address and port.",
    ),
  );
