...
const SERVER_STYLE_ID = "krot-server-styles";
...
function randomFreePort() {
  const used = new Set(
    (uci.sections(UCI_PACKAGE, "server") || [])
      .map((section) => `${uci.get(UCI_PACKAGE, section[".name"] || section, "listen_port") || ""}`)
      .filter(Boolean),
  );
...
function defaultTlsCertificatePath(sectionId) {
  return `/etc/krot/server-certs/${serverSafeName(sectionId)}.crt`;
}

function defaultTlsKeyPath(sectionId) {
  return `/etc/krot/server-certs/${serverSafeName(sectionId)}.key`;
}
...
function ensureProtocolDefaults(sectionId, protocol, forceProtocolDefaults) {
  ...
  setDefault(sectionId, "listen_port", randomFreePort());
...
function generateRealityKeypair(sectionId) {
  return fs
    .exec("/usr/bin/krot", ["generate_reality_keypair"])
    .then((response) => {
...
function generateRealityKeypairIfMissing(sectionId) {
  if (hasRealityKeypair(sectionId)) {
    return Promise.resolve();
  }
  ...
  if (
    ...
    return generateRealityKeypair(sectionId).then(() => {
      if (!getFirstListValue(sectionId, "reality_short_id")) {
        setDefault(sectionId, "reality_short_id", randomHex(4));
      }
    });
  }
...
function createServerContent(section, options = {}) {
  ...
  o.load = function (sectionId) {
    return loadDefaultPublicHost().then(
      (host) =>
        uci.get(UCI_PACKAGE, sectionId, "public_host") || host || "",
    );
  };

  o = section.option(form.ListValue, "routing_mode", _("Routing mode"));
  o.value("rules", _("K.R.O.T. rules"));
