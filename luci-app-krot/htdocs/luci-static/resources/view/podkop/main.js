...
async function onMount(target, timeoutMs = 10000) {
...
// src/icons/renderUploadIcon24.ts
function renderUploadIcon24() {
  const NS = "http://www.w3.org/2000/svg";
  return svgEl(
    "svg",
    {
...
function renderSubscriptionMetadata(metadata, extraActions = []) {
...
function renderSubscriptionUpdateAction(section, subscriptionUpdating, onUpdateSubscription) {
  ...
  return E(
    ...
    {
      type: "button",
      class: "btn cbi-button pdk_dashboard-page__outbound-section__subscription-update",
...
function renderDefaultState({
  ...
  function renderOutbound(outbound) {
    ...
    const canChooseOutbound = sectionEnabled && section.withTagSelect && !selectorSwitching && !outbound.selected;
    const className = [
      "ifacebox",
      ...
      !sectionEnabled || section.withTagSelect && !canChooseOutbound ? "pdk_dashboard-page__outbound-grid__item--disabled" : "",
    ...
    return E(
      "div",
      {
        ...
        "aria-disabled": !sectionEnabled || section.withTagSelect && !canChooseOutbound ? "true" : void 0,
        ...
        E("div", { class: "ifacebox-head center pdk_dashboard-page__outbound-grid__item__header", style: "font-weight:bold;text-align:center" }, [
          ...
          ...canCopyLink ? [
            E(
              ...
              {
                type: "button",
                class: "btn cbi-button pdk_dashboard-page__outbound-grid__item__copy-button",
  ...
  const metadataNodes = (section.subscriptionMetadata || []).map(
    (metadata, index) => renderSubscriptionMetadata(
      metadata,
      index === 0 && subscriptionUpdateAction ? [subscriptionUpdateAction] : []
    )
  ).filter(Boolean);
  const selectedOutbound = section.outbounds.find((outbound) => outbound.selected) || section.outbounds[0];
  const subscriptionTitle = section.subscriptionMetadata?.find((metadata) => metadata.title)?.title || section.displayName;
  function renderSubscriptionSummary() {
    ...
    const latencyClass = !latency ? "pdk_dashboard-page__outbound-grid__item__latency--empty" : latency < 800 ? "pdk_dashboard-page__outbound-grid__item__latency--green" : latency < 1500 ? "pdk_dashboard-page__outbound-grid__item__latency--yellow" : "pdk_dashboard-page__outbound-grid__item__latency--red";
    return E(
      "div",
      {
      ...
      [
        E(
          "div",
          {
            class: "ifacebox-head center pdk_dashboard-page__outbound-grid__item__header",
        ...
        E(
          "div",
          {
            class: "ifacebox-head cbi-tooltip-container pdk_dashboard-page__outbound-grid__item__status",
            style: "display:flex"
          },
          [
            E("div", {
              class: "zonebadge",
              style: `cursor:help;flex:1;height:3px;opacity:1;--zone-color-rgb:${sectionEnabled ? "144, 240, 144" : "240, 144, 144"};background-color:rgb(var(--zone-color-rgb))`
            }),
            E(
              "span",
  ...
  function renderSubscriptionDetails() {
    ...
    let latencyRefreshRunning2 = false;
    function getSortedOutbounds(outbounds) {
      return [...outbounds].sort((left, right) => {
        const leftLatency = Number.isFinite(left.latency) && left.latency > 0 ? left.latency : Number.POSITIVE_INFINITY;
        const rightLatency = Number.isFinite(right.latency) && right.latency > 0 ? right.latency : Number.POSITIVE_INFINITY;
    ...
    function renderLatency(outbound) {
      const latency = outbound.latency;
      const latencyClass = !latency ? "pdk_dashboard-page__outbound-grid__item__latency--empty" : latency < 800 ? "pdk_dashboard-page__outbound-grid__item__latency--green" : latency < 1500 ? "pdk_dashboard-page__outbound-grid__item__latency--yellow" : "pdk_dashboard-page__outbound-grid__item__latency--red";
      return E("span", { class: latencyClass }, latency ? `${latency}ms` : "N/A");
    }
    function renderAction(outbound, closeOnSelect = false) {
      const sectionEnabled = currentSection.enabled !== false;
      const selectorSwitching = Boolean(selectorSwitchingTag);
      const outboundSwitching = selectorSwitchingTag === outbound.code;
      const canChooseOutbound = sectionEnabled && currentSection.withTagSelect && !selectorSwitching && !outbound.selected;
      const canCopyLink = Boolean(outbound.canCopyLink) || isCopyableProxyLink(outbound.link);
      const actions = [];
        ...
        actions.push(
          E(
            "button",
            {
              ...
              disabled: !canChooseOutbound ? true : void 0,
              click: (event) => {
                event.preventDefault();
                event.stopPropagation();
            ...
            outboundSwitching ? _("Switching...") : outbound.selected ? _("Selected") : _("Select")
          )
        );
      }
      if (canCopyLink) {
    ...
    function renderOutboundsTable() {
      const visibleOutbounds = latencyRefreshRunning2 ? currentSection.outbounds : getSortedOutbounds(currentSection.outbounds);
      return E("table", { class: "table pdk_dashboard-page__subscription-table" }, [
        ...
        ...visibleOutbounds.length ? visibleOutbounds.map((outbound, index) => {
          const countryFlag = renderCountryFlag(outbound.country);
          return E(
            "tr",
            {
    ...
    function openSubscriptionModal() {
      ...
      function mergeFreshSectionPreservingOrder(freshSection) {
      ...
      async function refreshLatency(event) {
        event?.preventDefault();
    ...
    const shareUrl = section.subscriptionUrls?.[0] || firstMetadata?.webPageUrl || firstMetadata?.supportUrl || firstMetadata?.announceUrl;
    const shareAction = renderMetadataAction(_("Share"), shareUrl);
    const subscriptionTrafficText = firstMetadata?.traffic ? firstMetadata.traffic.isUnlimited ? `${formatBytes(firstMetadata.traffic.used) || "0 B"} / \u221E` : `${formatBytes(firstMetadata.traffic.used) || "0 B"} / ${formatBytes(firstMetadata.traffic.total) || "0 B"}` : "";
    const subscriptionExpireText = firstMetadata?.expire ? formatDate(firstMetadata.expire) : "";
          ...
          E("tr", { class: "tr cbi-rowstyle-1" }, [
            E("td", { class: "td", "data-title": _("Name") }, [
              ...
              ...selectedOutbound ? [
                E("br"),
                E("small", {}, [
                  ...renderCountryFlag(selectedOutbound.country) ? [renderCountryFlag(selectedOutbound.country), " "] : [],
  ...
  const sectionCards = section.subscriptionSourceCount ? [renderSubscriptionSummary() || renderOutbound(fallbackOutbound)] : section.outbounds.length ? section.outbounds.map((outbound) => renderOutbound(outbound)) : [renderOutbound(fallbackOutbound)];
...
function renderFailedState2(title) {
  return E(
    "div",
    {
      id: "",
...
function renderLoadingState2(title) {
  return renderDefaultState2({
    loading: true,
    failed: false,
    title,
...
function renderWidget(props) {
  if (props.loading) {
    return props.items.length ? renderDefaultState2(props) : renderLoadingState2(props.title);
  }
  if (props.failed) {
    return renderFailedState2(props.title);
  }
...
async function callBaseMethod(method, args = [], command = "/usr/bin/krot", options = {}) {
  try {
...
((Podkop2) => {
  ...
  ((AvailableMethods2) => {
    ...
    AvailableMethods2["BACKUP_CONFIG"] = "backup_config";
...
var COMPONENT_ACTION_STATE_DIR = "/var/run/krot/component-actions";
function sleep(ms) {
...
var PodkopShellMethods = {
  ...
  getClashApiProxyLatency: async (tag, options = {}) => callBaseMethod(
  ...
  getClashApiGroupLatency: async (tag, options = {}) => callBaseMethod(
    Podkop.AvailableMethods.CLASH_API,
    [
  ...
  exportConfig: () => callBaseMethod(Podkop.AvailableMethods.BACKUP_CONFIG),
...
var DASHBOARD_SECTION_CACHE_DIR = "/var/run/krot/section-cache";
...
async function getDashboardSections(options = {}) {
  ...
  const data = await Promise.all(
    configSections.filter(
      (section) => ["direct", "proxy", "outbound", "vpn", "byedpi"].includes(
...
var TabService = class _TabService {
  ...
  getActiveTabId() {
    const active = Array.from(
      document.querySelectorAll(".cbi-tab:not(.cbi-tab-disabled)")
...
function buildRouteDisplayNames(sections) {
  ...
  sections.filter((section) => section[".type"] === "section").forEach((section) => {
    const sectionName = section[".name"];
  ...
  store.subscribe(
    (next, prev, diff) => {
      if (diff.tabService && next.tabService.current !== prev.tabService.current) {
        const isMonitoringVisible = next.tabService.current === "monitoring" || next.tabService.current === "dashboard";
...
async function initController(controllerDependencies = {}) {
  dependencies = {
    ...dependencies,
  ...
  onMount("monitoring-status").then(() => {
    ...
    if (store.get().tabService.current === "monitoring" || store.get().tabService.current === "dashboard") {
      onPageMount();
    }
  });
}
...
var LATENCY_REFRESH_INTERVAL_MS = 1e3;
var SECTION_TRAFFIC_REFRESH_INTERVAL_MS = 5e3;
var sectionsRefreshTimer = null;
var latencyRefreshTimer = null;
var sectionsRefreshPromise = null;
var sectionsRefreshQueued = false;
var latencyRefreshRunning = false;
var dashboardMounted = false;
var dashboardMountId = 0;
var lastSectionTrafficUpdateAt = 0;
var lastSectionTraffic = {};
function applyLastSectionTraffic(sections) {
...
async function connectToClashSockets() {
  ...
  socket.subscribe(
    ...
    (msg) => {
      ...
      const now = Date.now();
      if (now - lastSectionTrafficUpdateAt >= SECTION_TRAFFIC_REFRESH_INTERVAL_MS) {
...
function getRouteTagFromRule2(rule) {
  const match = `${rule || ""}`.trim().match(/=>\s*route\(([^)]+)\)/);
  return `${match?.[1] || ""}`.trim().replace(/^['"]|['"]$/g, "");
}
function normalizeTrafficKey(value) {
  return `${value || ""}`.trim().toLowerCase().replace(/-out$/, "");
}
function getSectionTrafficTags(section) {
  const sectionName = section.sectionName;
  const displayName = section.displayName;
  if (section.action === "direct") {
    return ["direct", "direct-out", sectionName, displayName].map(normalizeTrafficKey).filter(Boolean);
  }
  return [
    section.code,
    sectionName,
  ...
  ].map(normalizeTrafficKey).filter(Boolean);
}
function updateSectionTraffic(payload) {
  const sectionsWidget = store.get().sectionsWidget;
  ...
  const nextData = sectionsWidget.data.map((section) => {
    ...
    connections.forEach((connection) => {
      const chains = Array.isArray(connection.chains) ? connection.chains : [];
      const routeTag = getRouteTagFromRule2(connection.rule);
      const matched = [...chains, routeTag].map(normalizeTrafficKey).some((tag) => tags.has(tag));
      if (!matched) {
        return;
      }
...
async function handleTestGroupLatency(sectionName, tag, options = {}) {
  if (store.get().sectionsWidget.latencyFetchingSections[sectionName] && !options.force) {
    return store.get().sectionsWidget.data.find((section) => section.sectionName === sectionName);
  }
  ...
  try {
    ...
    try {
      result = await withTimeout(
        ...
        options.quick ? 8e3 : 3e4,
        `dashboard group latency ${tag}`
      );
    } catch (error) {
      logger.warn(
        "[DASHBOARD]",
        `Group latency check timed out for ${tag}: ${error.message}`
      );
      return store.get().sectionsWidget.data.find((section) => section.sectionName === sectionName);
    }
    if (!result.success) {
...
async function handleTestProxyLatency(sectionName, tag, options = {}) {
  if (store.get().sectionsWidget.latencyFetchingSections[sectionName] && !options.force) {
    return store.get().sectionsWidget.data.find((section) => section.sectionName === sectionName);
  }
  setLatencyFetching(sectionName, true);
  try {
    ...
    try {
      result = await withTimeout(
        ...
        options.quick ? 4e3 : 15e3,
        `dashboard proxy latency ${tag}`
      );
    } catch (error) {
      logger.warn(
        "[DASHBOARD]",
        `Latency check timed out for ${tag}: ${error.message}`
      );
      return store.get().sectionsWidget.data.find((section) => section.sectionName === sectionName);
    }
    if (!result.success) {
...
function getDashboardLatencyTarget(section) {
...
function getDashboardOutboundLatencyTargets(sections) {
  return sections.flatMap((section) => {
    if (section.enabled === false) {
      return [];
    }
    return section.outbounds.filter((outbound) => outbound.showLatency !== false).filter((outbound) => outbound.code).map((outbound) => ({
      sectionName: section.sectionName,
      tag: outbound.code
    }));
  });
}
var OUTBOUND_LATENCY_CONCURRENCY = 12;
...
async function refreshDashboardLatencies() {
  ...
  const sectionTargets = sections.map(getDashboardLatencyTarget).filter(Boolean);
  ...
  try {
    await Promise.all(
      sectionTargets.map(
        (target) => target.group ? PodkopShellMethods.getClashApiGroupLatency(target.tag) : PodkopShellMethods.getClashApiProxyLatency(target.tag)
      )
    );
    await runWithConcurrency(
      outboundTargets.map(
        (target) => () => PodkopShellMethods.getClashApiProxyLatency(target.tag)
      ),
...
async function renderSectionsWidget() {
  ...
  if (sectionsWidget.loading || sectionsWidget.failed) {
    const renderedWidget = renderSections({
      ...
      onTestLatency: async () => void 0,
      onTestOutboundLatency: async () => void 0,
      onChooseOutbound: () => {
      },
      onCopyOutbound: () => {
      },
  ...
  const renderedWidgets = sectionsWidget.data.map(
    (section) => renderSections({
      ...
      onTestOutboundLatency: (tag, options) => handleTestProxyLatency(section.sectionName, tag, options),
  ...
  const renderedSubscriptionDetails = sectionsWidget.data.filter((section) => section.subscriptionSourceCount).map(
    (section) => renderSections({
      loading: sectionsWidget.loading,
      failed: sectionsWidget.failed,
      section,
...
#cbi-${PODKOP_UCI_PACKAGE}-dashboard-_mount_node.cbi-value {
    display: block;
    margin-bottom: 0;
}
...
async function runFakeIPCheck() {
  ...
  checks.browserFakeIP = checkFakeIPResponse.success && (checkFakeIPResponse.data.fakeip || checks.differentIP);
...
// src/podkop/tabs/diagnostic/partials/renderConfigBackup.ts
...
var configExportLoading = false;
var configImportLoading = false;
function sleep2(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}
