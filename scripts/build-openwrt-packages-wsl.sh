...
WSL_NATIVE_ROOT="${WSL_NATIVE_ROOT:-$DEFAULT_BUILD_HOME/build/krot}"
WORK_DIR="${WORK_DIR:-$ROOT_DIR/.wsl-build}"
SDK_WORK_DIR="${SDK_WORK_DIR:-$WORK_DIR/sdk}"
SDK_CACHE_DIR="${SDK_CACHE_DIR:-$DEFAULT_BUILD_HOME/.cache/krot/openwrt-sdk}"
...
BACKEND_DEPENDS_IPK="libc, sing-box, curl, ucode, ucode-mod-fs, kmod-nft-tproxy, coreutils-base64, coreutils-sort, bind-dig, nftables, kmod-nft-nat, kmod-nft-offload"
BACKEND_DEPENDS_APK="bind-dig coreutils-base64 coreutils-sort curl ucode ucode-mod-fs kmod-nft-nat kmod-nft-offload kmod-nft-tproxy libc nftables sing-box"
...
build_backend_root() {
  ...
  make_dir "$output_root/usr/lib/krot"
...
build_app_root() {
  ...
  if [[ -d "$ROOT_DIR/luci-app-krot/htdocs" ]]; then
    cp -a "$ROOT_DIR/luci-app-krot/htdocs/." "$output_root/www/"
  fi

  if [[ -d "$output_root/www/luci-static/resources/view/podkop" ]]; then
    rm -rf "$output_root/www/luci-static/resources/view/krot"
    mv "$output_root/www/luci-static/resources/view/podkop" \
      "$output_root/www/luci-static/resources/view/krot"
  fi

  if [[ -d "$ROOT_DIR/luci-app-krot/root" ]]; then
    cp -a "$ROOT_DIR/luci-app-krot/root/." "$output_root/"
  fi

  if [[ -f "$output_root/www/luci-static/resources/view/krot/main.js" ]]; then
    sed -i -e "s/__COMPILED_VERSION_VARIABLE__/${RELEASE_VERSION}/g" \
      "$output_root/www/luci-static/resources/view/krot/main.js"
  fi
...
build_i18n_root() {
  local output_root="$1"
  local po2lmo_bin="$2"
  local lmo_path="$output_root/usr/lib/lua/luci/i18n/krot.ru.lmo"

  rm -rf "$output_root"
  make_dir "$output_root/etc/uci-defaults"
  make_dir "$(dirname "$lmo_path")"

  cat > "$output_root/etc/uci-defaults/luci-i18n-krot-ru" <<'EOF'
uci set luci.languages.ru='Русский (Russian)'; uci commit luci
EOF

  "$po2lmo_bin" "$ROOT_DIR/luci-app-krot/po/ru/krot.po" "$lmo_path"
...
EOF
  ...
  chmod 0755 "$control_dir/preinst" "$control_dir/postinst" "$control_dir/prerm"
}
...
write_i18n_ipk_control() {
  ...
  cat > "$control_dir/control" <<EOF
Package: luci-i18n-krot-ru
Version: ${RELEASE_VERSION}
Depends: libc, luci-app-krot
License: GPL-2.0-or-later
Section: luci
URL: ${PROJECT_URL}
Maintainer: ${MAINTAINER}
...
. ${IPKG_INSTROOT}/lib/functions.sh
export root="${IPKG_INSTROOT}"
export pkgname="luci-i18n-krot-ru"
add_group_and_user
default_postinst
EOF
...
sync_artifacts_to_windows() {
  ...
  rm -f \
    "$WINDOWS_ARTIFACTS_DIR"/krot_* \
    "$WINDOWS_ARTIFACTS_DIR"/luci-app-krot_* \
    "$WINDOWS_ARTIFACTS_DIR"/luci-i18n-krot-ru_*

  cp -f \
    "$output_dir"/krot_"${RELEASE_VERSION}"_all.ipk \
    "$output_dir"/luci-app-krot_"${RELEASE_VERSION}"_all.ipk \
    "$output_dir"/luci-i18n-krot-ru_"${RELEASE_VERSION}"_all.ipk \
    "$output_dir"/krot_"${RELEASE_VERSION}".apk \
    "$output_dir"/luci-app-krot_"${RELEASE_VERSION}".apk \
    "$output_dir"/luci-i18n-krot-ru_"${RELEASE_VERSION}".apk \
...
main() {
  ...
  mkdir -p "$output_dir"
  rm -f "$output_dir"/krot_* "$output_dir"/luci-app-krot_* "$output_dir"/luci-i18n-krot-ru_*
  ...
  generate_apk_metadata_files "krot" "$backend_root" "/etc/config/krot"
  generate_apk_metadata_files "luci-app-krot" "$app_root"
  generate_apk_metadata_files "luci-i18n-krot-ru" "$i18n_root"
  ...
  verify_ipk_metadata "$output_dir/krot_${RELEASE_VERSION}_all.ipk" "krot" "$RELEASE_VERSION"
  verify_ipk_metadata "$output_dir/luci-app-krot_${RELEASE_VERSION}_all.ipk" "luci-app-krot" "$RELEASE_VERSION"
  verify_ipk_metadata "$output_dir/luci-i18n-krot-ru_${RELEASE_VERSION}_all.ipk" "luci-i18n-krot-ru" "$RELEASE_VERSION"
  verify_apk_metadata "$apk_bin" "$output_dir/krot_${RELEASE_VERSION}.apk" "krot" "$APK_INTERNAL_VERSION"
  verify_apk_metadata "$apk_bin" "$output_dir/luci-app-krot_${RELEASE_VERSION}.apk" "luci-app-krot" "$APK_INTERNAL_VERSION"
  verify_apk_metadata "$apk_bin" "$output_dir/luci-i18n-krot-ru_${RELEASE_VERSION}.apk" "luci-i18n-krot-ru" "$APK_INTERNAL_VERSION"
