...
hub_get_modules() {
    ...
    for module_id in $module_ids; do
        module_id="$(echo "$module_id" | tr -d '[:space:]')"
...
hub_remove_module() {
    ...
    updates_init_tmp_dir || updates_fail "hub" "hub_remove_${module_id}" "Failed to create temp dir"

    # Determine package name and cleanup for known modules
    case "$module_id" in
        zapret)
            pkg_name="zapret"
