...
hub_get_modules() {
    ...
    local module_json module_component installed installed_version first=1 result="["
    ...
    module_ids="$(grep -o '"modules"[[:space:]]*:[[:space:]]*\[' "$tmp_index" 2>/dev/null | head -1)"
    ...
    for module_id in $module_ids; do
        ...
        if [ -s "$tmp_module" ]; then
            ...
            if hub_module_installed_status "$module_component"; then
                installed_json=",\"installed\":true,\"installed_version\":\"${hub_installed_version//\"/\\\\\"}\""
            else
...
hub_add_source() {
    ...
    local tmp_json="$UPDATES_TMP_DIR/source-index.json"
...
hub_remove_module() {
    ...
    hub_repo="$(uci -q get krot.settings.hub_repo 2>/dev/null || true)"
    hub_repo="${hub_repo:-titovcode/krot}"
    branch="$(uci -q get krot.settings.hub_branch 2>/dev/null || true)"
    branch="${branch:-main}"
