...
component_action_async() {
    ...
    (
        trap '' HUP
        /usr/bin/krot component_action "$component" "$action" >"$output_file" 2>&1
...
updates_check_krot() {
    ...
    if [ "$latest_version" = "unknown" ]; then
        updates_fail "podkop" "check_update" "Failed to check K.R.O.T. updates" "$PODKOP_VERSION" "$latest_version"
    fi
    ...
    latest)
        message="Latest version is installed"
        updates_log "K.R.O.T. is already up to date ($PODKOP_VERSION)"
        ;;
    outdated)
        message="Update is available"
    ...
    dev)
        message="Installed version is newer than release"
        updates_log "K.R.O.T. installed version is newer than upstream release: $PODKOP_VERSION -> $latest_version"
        ;;
    esac
