...
function collapse_range_to_number(value) {
    value = as_string(value);
    if (value == "")
        return null;
    let dash = index(value, "-");
    if (dash < 0)
        return is_integer_string(value) ? int(value, 10) : null;
    let lo = substr(value, 0, dash);
    let hi = substr(value, dash + 1);
    if (!is_integer_string(lo) || !is_integer_string(hi))
        return null;
    return lo == hi ? int(lo, 10) : value;
}

function extract_xhttp_extra(query) {
    let extra_raw = query.extra || "";
    if (extra_raw == "")
        return {};

    let extra;
    try { extra = json(extra_raw); } catch (e) { return {}; }
    if (type(extra) != "object")
        return {};

    let xhttp_extra = null;
    if (type(extra.xhttpSettings) == "object" && type(extra.xhttpSettings.extra) == "object")
        xhttp_extra = extra.xhttpSettings.extra;
    if (!xhttp_extra && type(extra.downloadSettings) == "object" && type(extra.downloadSettings.xhttpSettings) == "object" && type(extra.downloadSettings.xhttpSettings.extra) == "object")
        xhttp_extra = extra.downloadSettings.xhttpSettings.extra;
    if (!xhttp_extra)
        return {};

    let result = {};
    let pb = as_string(xhttp_extra.xPaddingBytes || "");
    if (pb != "") {
        let collapsed = collapse_range_to_number(pb);
        if (collapsed != null)
            result.x_padding_bytes = collapsed;
    }
    let ngrpc = xhttp_extra.noGRPCHeader;
    if (ngrpc == true || ngrpc == "true")
        result.no_grpc_header = true;
    else if (ngrpc == false || ngrpc == "false")
        result.no_grpc_header = false;
    let sc_max = as_string(xhttp_extra.scMaxEachPostBytes || "");
    if (sc_max != "") {
        let collapsed = collapse_range_to_number(sc_max);
        if (collapsed != null)
            result.sc_max_each_post_bytes = collapsed;
    }
    let sc_min = as_string(xhttp_extra.scMinPostsIntervalMs || "");
    if (sc_min != "") {
        let collapsed = collapse_range_to_number(sc_min);
        if (collapsed != null)
            result.sc_min_posts_interval_ms = collapsed;
    }
    return result;
}
...
function add_transport(url) {
    ...
    if (transport == "xhttp") {
        ...
        if (host == "")
            host = sni;
        let xhttp_params = extract_xhttp_extra(query);
        let result = {
            type: "xhttp",
            mode: xhttp_mode,
            path: path,
            x_padding_bytes: xhttp_params.x_padding_bytes || "100-1000",
            no_grpc_header: xhttp_params.no_grpc_header != null ? xhttp_params.no_grpc_header : false,
            sc_max_each_post_bytes: xhttp_params.sc_max_each_post_bytes || 1000000,
            sc_min_posts_interval_ms: xhttp_params.sc_min_posts_interval_ms || 30
        };
        if (host != "")
            result.host = host;
        return result;
