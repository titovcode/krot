...
PODKOP_LIB="${PODKOP_LIB:-/usr/lib/krot}"
. "$PODKOP_LIB/helpers.sh"
. "$PODKOP_LIB/sing_box_config_manager.sh"

sing_box_cf_ucode() {
    ucode "${PODKOP_LIB:-/usr/lib/krot}/sing_box_config_facade.uc" "$@"
}
...
sing_box_cf_ucode_input() {
    ...
    printf '%s' "$input" |
        ucode "${PODKOP_LIB:-/usr/lib/krot}/sing_box_config_facade.uc" "$operation" "$@"
}
...
_add_outbound_transport() {
    ...
    xhttp)
        ...
        local xhttp_padding xhttp_nogrpc xhttp_sc_max xhttp_sc_min
        ...
        xhttp_padding=$(url_get_query_param "$url" "xPaddingBytes")
        xhttp_nogrpc=$(url_get_query_param "$url" "noGRPCHeader")
        xhttp_sc_max=$(url_get_query_param "$url" "scMaxEachPostBytes")
        xhttp_sc_min=$(url_get_query_param "$url" "scMinPostsIntervalMs")

        if [ -z "$xhttp_padding" ] || [ "$xhttp_padding" = "" ]; then
            local extra_raw extra_padding extra_nogrpc extra_sc_max extra_sc_min
            extra_raw=$(url_get_query_param "$url" "extra")
            if [ -n "$extra_raw" ]; then
                extra_padding=$(echo "$extra_raw" | jsonfilter -e '@.xhttpSettings.extra.xPaddingBytes' 2>/dev/null || echo "")
                extra_nogrpc=$(echo "$extra_raw" | jsonfilter -e '@.xhttpSettings.extra.noGRPCHeader' 2>/dev/null || echo "")
                extra_sc_max=$(echo "$extra_raw" | jsonfilter -e '@.xhttpSettings.extra.scMaxEachPostBytes' 2>/dev/null || echo "")
                extra_sc_min=$(echo "$extra_raw" | jsonfilter -e '@.xhttpSettings.extra.scMinPostsIntervalMs' 2>/dev/null || echo "")
                if [ -z "$extra_padding" ]; then
                    extra_padding=$(echo "$extra_raw" | jsonfilter -e '@.downloadSettings.xhttpSettings.extra.xPaddingBytes' 2>/dev/null || echo "")
                    extra_nogrpc=$(echo "$extra_raw" | jsonfilter -e '@.downloadSettings.xhttpSettings.extra.noGRPCHeader' 2>/dev/null || echo "")
                    extra_sc_max=$(echo "$extra_raw" | jsonfilter -e '@.downloadSettings.xhttpSettings.extra.scMaxEachPostBytes' 2>/dev/null || echo "")
                    extra_sc_min=$(echo "$extra_raw" | jsonfilter -e '@.downloadSettings.xhttpSettings.extra.scMinPostsIntervalMs' 2>/dev/null || echo "")
                fi
                [ -n "$extra_padding" ] && xhttp_padding="$extra_padding"
                [ -n "$extra_nogrpc" ] && xhttp_nogrpc="$extra_nogrpc"
                [ -n "$extra_sc_max" ] && xhttp_sc_max="$extra_sc_max"
                [ -n "$extra_sc_min" ] && xhttp_sc_min="$extra_sc_min"
            fi
        fi

        config=$(
            sing_box_cm_set_xhttp_transport_for_outbound "$config" "$outbound_tag" "$xhttp_path" "$xhttp_host" "$xhttp_mode" "$xhttp_padding" "$xhttp_nogrpc" "$xhttp_sc_max" "$xhttp_sc_min"
        )
        ;;
    *)
