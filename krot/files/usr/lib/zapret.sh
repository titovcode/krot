...
get_nfqws_unsupported_token_reason() {
    ...
    "<HOSTLIST>" | "<HOSTLIST_NOAUTO>")
        echo "K.R.O.T. does not expand zapret hostlist templates in per-rule strategies because sing-box already selects the resources before NFQUEUE."
        return 0
        ;;
    --hostlist | --hostlist=* | --hostlist-domains | --hostlist-domains=* | \
        ...
        echo "Hostname-based selection inside nfqws is incompatible with the K.R.O.T. architecture because sing-box already chooses which resources enter action=zapret."
        return 0
        ;;
    --ipset | --ipset=* | --ipset-ip | --ipset-ip=* | --ipset-exclude | --ipset-exclude=* | --ipset-exclude-ip | --ipset-exclude-ip=*)
        echo "IP or CIDR selection inside nfqws is incompatible with the K.R.O.T. architecture because sing-box already chooses which resources enter action=zapret."
        return 0
        ;;
    --qnum | --qnum=*)
        echo "The NFQUEUE number is assigned by K.R.O.T. per rule and must not be overridden in the strategy."
        return 0
        ;;
    --dpi-desync-fwmark | --dpi-desync-fwmark=*)
        echo "The desync fwmark is managed by K.R.O.T. for loop prevention and must not be overridden in the strategy."
        return 0
        ;;
    --daemon)
        echo "K.R.O.T. manages the nfqws process lifecycle itself. The strategy must not daemonize nfqws."
        return 0
        ;;
    --dry-run)
...
check_nfqws_strategy() {
    ...
    while [ "$#" -gt 0 ]; do
        ...
        if [ "$token_count" -eq 0 ]; then
            ...
            @* | \$*)
                set_nfqws_validation_failure \
                    "Unsupported NFQWS token '$token': External nfqws config files bypass K.R.O.T. validation and queue management." \
                    "$token"
                return 1
                ;;
            esac
...
get_zapret_status_json() {
    ...
    if [ "$configured" -eq 1 ] && [ "$provider_available" -eq 0 ]; then
        ...
        status_message="external NFQUEUE rules overlap with the K.R.O.T. zapret range $ZAPRET_QUEUE_BASE-$queue_range_end"
        ...
        status_message="unexpected K.R.O.T.-managed nfqws processes are running without matching action=zapret rules"
    elif [ "$configured" -eq 1 ] && [ "$ready" -eq 0 ]; then
        status_message="action=zapret is configured, but the K.R.O.T.-managed nfqws runtime is not ready"
    elif [ "$standalone_conflict" -eq 1 ]; then
        status_message="standalone zapret is active together with K.R.O.T. action=zapret; queues are separate, but packet-level policy overlap is possible"
