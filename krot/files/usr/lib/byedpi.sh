...
byedpi_reject_controlled_option() {
    ...
    --ip | --ip=* | -i | -i?* | --port | --port=* | -p | -p?*)
        ...
        set_byedpi_validation_failure "ByeDPI listen address and port are assigned by K.R.O.T. and must not be set in the strategy: $display" "$base_token"
        return 0
        ;;
    --transparent | -E | -E?*)
        set_byedpi_validation_failure "Transparent proxy mode is incompatible with action=byedpi because K.R.O.T. connects to ciadpi through SOCKS." "$base_token"
        return 0
        ;;
    --daemon | -D | -D?*)
        set_byedpi_validation_failure "K.R.O.T. manages the ciadpi process lifecycle itself, so daemon mode is not allowed here." "$base_token"
        return 0
        ;;
    --pidfile | --pidfile=* | -w | -w?*)
        set_byedpi_validation_failure "K.R.O.T. manages ciadpi pid files itself, so pidfile options are not allowed here." "$base_token"
        return 0
        ;;
...
start_byedpi_runtime() {
    ...
    if is_byedpi_standalone_service_enabled; then
        log "Standalone byedpi service is enabled. K.R.O.T. manages ciadpi itself for action 'byedpi'; disable standalone byedpi autostart to avoid boot-time port conflicts." "warn"
    fi

    if is_byedpi_standalone_service_running; then
        log "Stopping standalone byedpi service before starting K.R.O.T.-managed ciadpi runtime"
...
get_byedpi_status_json() {
    ...
    if [ "$configured" -eq 1 ] && [ "$provider_available" -eq 0 ]; then
        ...
        status_message="standalone byedpi service is active together with K.R.O.T. action=byedpi; port conflicts are possible"
        conflict=1
    elif [ "$configured" -eq 1 ] && [ "$standalone_service_enabled" -eq 1 ]; then
        status_message="standalone byedpi service autostart is enabled; disable it to avoid boot-time port conflicts with K.R.O.T. action=byedpi"
    elif [ "${running_process_count:-0}" -gt "${expected_process_count:-0}" ] ||
        [ "${supervisor_process_count:-0}" -gt "${expected_process_count:-0}" ]; then
        status_message="unexpected K.R.O.T.-managed ciadpi processes are running without matching action=byedpi rules"
    elif [ "$configured" -eq 1 ] && [ "$runtime_unstable" -eq 1 ]; then
        status_message="K.R.O.T.-managed ciadpi has restarted after exiting; the ByeDPI strategy or traffic load may be unstable"
    elif [ "$configured" -eq 1 ] && [ "$ready" -eq 0 ]; then
        status_message="action=byedpi is configured, but the K.R.O.T.-managed ciadpi runtime is not ready"
