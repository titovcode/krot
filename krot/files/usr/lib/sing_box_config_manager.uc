...
function set_xhttp_transport(config, args) {
    ...
    let transport = {
        ...
        x_padding_bytes: as_string(args[5]) != "" ? args[5] : "100-1000",
        no_grpc_header: args[6] == "true" ? true : (args[6] == "false" ? false : false),
        sc_max_each_post_bytes: as_string(args[7]) != "" ? json_arg(args[7]) : 1000000,
        sc_min_posts_interval_ms: as_string(args[8]) != "" ? json_arg(args[8]) : 30
    };
    optional_string(transport, "host", args[2]);
    patch_outbound(config, as_string(args[0]), { transport });
}
