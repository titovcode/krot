...
create_source_rule_set() {
    ...
    ucode "${PODKOP_LIB:-/usr/lib/krot}/rulesets.uc" create-source "$ruleset_filepath"
}
...
patch_source_ruleset_rules() {
    ...
    if ! ucode "${PODKOP_LIB:-/usr/lib/krot}/rulesets.uc" patch-source "$tmpfile" "$key" "$value"; then
        rm -f "$tmpfile"
        return 1
    fi
...
extract_ip_cidr_from_json_ruleset_to_file() {
    ...
    ucode "${PODKOP_LIB:-/usr/lib/krot}/rulesets.uc" extract-ip-cidr "$json_file" "$output_file"
}
...
extract_ip_cidr_nft_elements_from_json_ruleset_to_files() {
    ...
    ucode "${PODKOP_LIB:-/usr/lib/krot}/rulesets.uc" extract-ip-cidr-nft \
