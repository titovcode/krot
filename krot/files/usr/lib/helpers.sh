...
is_ipv4_cidr() {
    local ip="$1"
    local regex="^((25[0-5]|(2[0-4]|1\d|[1-9]|)\d)\.?\b){4}(\/(3[0-2]|[12]?\d?))$"
    [[ "$ip" =~ $regex ]]
}
...
setup_vpn_policy_routing() {
    ...
    gw_ip="$(ip -4 addr show dev "$interface_name" 2>/dev/null | grep -oP 'inet \K[0-9.]+' | head -1)"
    [ -n "$gw_ip" ] || {
        log "Cannot determine IP for interface '$interface_name'; skipping VPN policy routing for '$section'" "warn"
        return 1
    }
    ...
    ip route add default via "$gw_ip" dev "$interface_name" table "$table_name" 2>/dev/null || {
