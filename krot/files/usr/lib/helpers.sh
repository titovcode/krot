...
download_subscription() {
    ...
    # Reject values that would alter the curl/wget command line.
    case "$url" in
    http://* | https://*) ;;
    *) log "Invalid subscription URL scheme" "error"; return 1 ;;
    esac
    if [ -n "$http_proxy_address" ]; then
        case "$http_proxy_address" in
        *[!A-Za-z0-9.:-]*)
            log "Invalid subscription proxy address" "error"
            return 1
            ;;
        esac
    fi
...
json_utils_ucode() {
    ucode "${PODKOP_LIB:-/usr/lib/krot}/json_utils.uc" "$@"
}
...
provider_status_ucode() {
    ucode "${PODKOP_LIB:-/usr/lib/krot}/provider_status.uc" "$@"
}
...
_count_vpn_sections_handler() {
    ...
    [ "$VPN_SECTION_FOUND" = "1" ] && return 0

    config_get action "$section" "action"
    [ "$action" = "vpn" ] || return 0
    VPN_SECTION_COUNTER=$((VPN_SECTION_COUNTER + 1))
    [ "$section" = "$target" ] && VPN_SECTION_FOUND=1
}
...
get_vpn_mark() {
    ...
    index="$(vpn_section_index "$section")"
    printf '0x%x' "$((NFT_VPN_MARK_BASE + index))"
}
...
get_vpn_rt_table_id() {
    ...
    index="$(vpn_section_index "$section")"
    echo "$((NFT_VPN_RT_TABLE_BASE + index))"
}

get_vpn_rt_table_name() {
    local section="$1"
...
setup_vpn_policy_routing() {
    ...
    local mark table_id table_name
    local gw_ip route_added

    mark="$(get_vpn_mark "$section")"
    table_id="$(get_vpn_rt_table_id "$section")"
    table_name="$(get_vpn_rt_table_name "$section")"
    ...
    if [ -n "$gw_ip" ]; then
        ip route add default via "$gw_ip" dev "$interface_name" table "$table_name" 2>/dev/null && route_added=1 || {
            log "Failed to add default route via '$gw_ip' for VPN section '$section' on '$interface_name'; trying device-only route" "debug"
        }
    else
        log "No IPv4 gateway address for interface '$interface_name'; using device-only default route for VPN section '$section'" "debug"
    fi

    if [ "$route_added" -eq 0 ]; then
        ip route add default dev "$interface_name" table "$table_name" 2>/dev/null || {
            log "Failed to add default route for VPN section '$section' via $interface_name" "warn"
            return 1
        }
    fi

    ip rule del fwmark "$mark" table "$table_name" 2>/dev/null || true
    ip -4 rule add fwmark "$mark" table "$table_name" priority "$((1000 + table_id))" 2>/dev/null || {
        log "Failed to add ip rule for VPN section '$section'" "warn"
        return 1
    }

    log "VPN policy routing set up: section='$section' interface='$interface_name' mark=$mark table=$table_name" "debug"
    return 0
}

cleanup_vpn_policy_routing() {
    local section="$1"
    local mark table_id table_name
    ...
    ip rule del fwmark "$mark" table "$table_name" 2>/dev/null || true
    ip route flush table "$table_name" 2>/dev/null || true
    sed -i "/^$table_id $table_name$/d" /etc/iproute2/rt_tables 2>/dev/null || true
}

cleanup_all_vpn_policy_routing() {
    local section

    config_foreach _cleanup_vpn_section_handler "section"
}

_cleanup_vpn_section_handler() {
    local section="$1"
    local action

    config_get action "$section" "action"
    [ "$action" = "vpn" ] || return 0
    cleanup_vpn_policy_routing "$section"
}
