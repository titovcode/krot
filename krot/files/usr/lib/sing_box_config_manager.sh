...
sing_box_cm_ucode() {
    ...
    printf '%s' "$config" |
        ucode "${PODKOP_LIB:-/usr/lib/krot}/sing_box_config_manager.uc" "$operation" "$@"
}
...
sing_box_cm_save_config_to_file() {
    ...
    printf '%s' "$config" |
        ucode "${PODKOP_LIB:-/usr/lib/krot}/sing_box_config_manager.uc" save-config "$filepath"
}

_normalize_arg() {
    ucode "${PODKOP_LIB:-/usr/lib/krot}/sing_box_config_manager.uc" normalize-arg "$1"
}
