...
const LOG_NOTIFICATION_DEDUPE_WINDOW_MS = 15_000;
...
function isLogLifecycleBoundary(line: string) {
  const lower = line.toLowerCase();
  return (
    lower.includes('[info] starting k.r.o.t.') ||
    lower.includes('[info] stopping k.r.o.t.') ||
    lower.includes('[info] k.r.o.t. reload') ||
    lower.includes('[info] k.r.o.t. restart')
  );
}
...
export function coreService() {
  ...
  // Immediate keepalive + periodic to prevent LuCI session expiry
  PodkopShellMethods.getSystemInfo().catch(() => {});
  setInterval(() => {
    PodkopShellMethods.getSystemInfo().catch(() => {});
