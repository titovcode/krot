...
function getComponentCards(): ComponentCard[] {
  ...
  Object.entries(hubModulesStore).forEach(([key, moduleData]) => {
    ...
    const displayVersion = systemInfoLoading
      ? 'loading'
      : installed
        ? version || _('Installed')
        : _('Not installed');
...
function renderUpdatesComponents() {
  ...
  return preserveScrollForPage(() => {
    ...
    container.replaceChildren(...moduleCards.map(renderComponentCard));
  });
}
...
async function autoCheckKrot() {
  if (
    autoCheckKrotInProgress ||
    store.get().updatesChecks.podkop.status !== null
  ) {
    return;
  }
