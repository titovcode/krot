...
let configExportLoading = false;
let configImportLoading = false;
...
let cachedEnabledActions: Set<string> | null = null;

async function refreshEnabledActionsCache() {
  try {
    const sections = await getConfigSections();
    cachedEnabledActions = new Set(
      sections
        .filter((s) => s.enabled !== '0')
        .map((s) => ((s.action as string) || '').toLowerCase()),
    );
  } catch {
    cachedEnabledActions = null;
  }
}
...
function getDiagnosticsProviderOptions(
  ...
  const hasZapretSection = cachedEnabledActions
    ? cachedEnabledActions.has('zapret')
    : true;
  const hasByedpiSection = cachedEnabledActions
    ? cachedEnabledActions.has('byedpi')
    : true;

  return {
    includeZapret: Boolean(systemInfo.zapret_installed) && hasZapretSection,
    includeByedpi: Boolean(systemInfo.byedpi_installed) && hasByedpiSection,
...
function renderDiagnosticSystemInfoWidget() {
  ...
  if (diagnosticsSystemInfo.adguard_installed) {
    items.push({
      key: 'AdGuard Home',
      value: diagnosticsSystemInfo.adguard_version,
    });
  }
...
function renderConfigBackupWidget() {
  const container = document.getElementById(
    'pdk_diagnostic-page-config-backup',
  );
  if (!container) return;

  container.replaceChildren(
    renderConfigBackup({
      exportLoading: configExportLoading,
      importLoading: configImportLoading,
...
async function handleExportConfig() {
  if (configExportLoading) return;
  configExportLoading = true;
  renderConfigBackupWidget();

  try {
    const result = await PodkopShellMethods.exportConfig();
    if (result.success && result.data) {
      const date = new Date().toISOString().slice(0, 10);
      downloadAsTxt(result.data, `krot-config-${date}.cfg`);
    } else {
      showToast(_('Failed to export config'), 'error');
    }
  } finally {
...
async function handleImportConfig(config: string) {
  if (configImportLoading) return;
  configImportLoading = true;
  renderConfigBackupWidget();

  try {
    const result = await PodkopShellMethods.importConfig(config);
    if (result.success) {
      showToast(_('Config imported. Restart K.R.O.T. to apply.'), 'success');
    } else {
      showToast(_('Failed to import config'), 'error');
    }
  } finally {
...
async function runChecks() {
  try {
    ...
    await refreshEnabledActionsCache();
...
function onPageMount() {
  ...
  // Pre-fetch enabled actions to avoid flicker in initial state
  void refreshEnabledActionsCache().then(() => renderDiagnosticsChecks());
