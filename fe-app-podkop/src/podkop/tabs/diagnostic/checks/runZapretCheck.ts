...
export async function runZapretCheck() {
  ...
  } else if (hasZapretRules && !podkopRuntimeReady) {
    state = 'error';
    description = _('Checks failed');
  ...
  const items: Array<IDiagnosticsChecksItem> = [
    ...
    },
    {
      ...
      key: hasZapretRules
        ? podkopRuntimeReady
          ? _('K.R.O.T.-managed nfqws runtime is ready')
          : _('K.R.O.T.-managed nfqws runtime is not ready')
        : unexpectedRuntime
          ? _('Unexpected K.R.O.T.-managed nfqws runtime is running')
          : _('K.R.O.T.-managed nfqws runtime is not running'),
      value: '',
    },
    {
    ...
    },
    {
      ...
      key: standaloneServiceRunning
        ? hasZapretRules
          ? _(
              'Standalone Zapret is active together with K.R.O.T. Zapret rules',
            )
