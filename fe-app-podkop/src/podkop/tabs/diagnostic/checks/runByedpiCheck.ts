...
export async function runByedpiCheck() {
  ...
  } else if (hasByedpiRules && !podkopRuntimeReady) {
    state = 'error';
    description = _('Checks failed');
  ...
  const items: Array<IDiagnosticsChecksItem> = [
    ...
    },
    {
      ...
      key: hasByedpiRules
        ? runtimeUnstable
          ? _('K.R.O.T.-managed ciadpi runtime has restarted')
          : podkopRuntimeReady
            ? _('K.R.O.T.-managed ciadpi runtime is ready')
            : _('K.R.O.T.-managed ciadpi runtime is not ready')
        : unexpectedRuntime
          ? _('Unexpected K.R.O.T.-managed ciadpi runtime is running')
          : _('K.R.O.T.-managed ciadpi runtime is not running'),
    ...
    },
    {
      ...
      key: standaloneServiceRunning
        ? hasByedpiRules
          ? _(
              'Standalone ByeDPI is active together with K.R.O.T. ByeDPI rules',
            )
