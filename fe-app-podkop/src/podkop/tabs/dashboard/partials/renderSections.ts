...
function renderDefaultState({
    ...
    return E(
      ...
      [
        ...(outboundSwitching
          ...
          : []),
        E('div', { class: 'ifacebox-head center pdk_dashboard-page__outbound-grid__item__header', style: 'font-weight:bold;text-align:center' }, [
        ...
        E('div', { class: 'ifacebox-body pdk_dashboard-page__outbound-grid__item__body' }, [
          E('img', {
        ...
        E('div', { class: 'ifacebox-head cbi-tooltip-container pdk_dashboard-page__outbound-grid__item__status', style: 'display:flex' }, [
          E('div', {
            class: 'zonebadge',
            style: `cursor:help;flex:1;height:3px;opacity:1;--zone-color-rgb:${getStatusColor()};background-color:rgb(var(--zone-color-rgb))`,
          }),
          E('span', { class: 'cbi-tooltip left' }, sectionEnabled ? _('Enabled') : _('Disabled')),
        ]),
        E('div', { class: 'ifacebox-body' }, [
          E('div', {
            class: 'pdk_dashboard-page__outbound-grid__item__stats cbi-tooltip-container',
            style: 'text-align:left;font-size:80%',
          }, [
            ...(showLatency
              ? [
                  E('span', { class: getLatencyClass() }, outbound.latency ? `${outbound.latency}ms` : 'N/A'),
                  E('br'),
                ]
              : []),
            `▲ ${prettyBytes(trafficUpload)}`,
  ...
  function renderSubscriptionSummary() {
    ...
    return E(
      'div',
      {
        class: 'ifacebox pdk_dashboard-page__outbound-grid__item pdk_dashboard-page__outbound-grid__item--active',
      },
      [
        E(
          'div',
        ...
        E(
          'div',
          {
            class: 'ifacebox-body pdk_dashboard-page__outbound-grid__item__body',
          },
          [
            E('img', {
              src: getSectionIconPath(section),
  ...
  function renderSubscriptionDetails() {
    ...
    function renderLatency(outbound: Podkop.Outbound) {
      ...
      return E('span', { class: latencyClass }, latency ? `${latency}ms` : 'N/A');
    }
    ...
    function renderOutboundsTable() {
      ...
      return E('table', { class: 'table pdk_dashboard-page__subscription-table' }, [
          ...
          ...(visibleOutbounds.length
                ...
                return E(
                  ...
                  [
                    E('td', { class: 'td', 'data-title': _('Name') }, [
                      ...(countryFlag ? [countryFlag, ' '] : []),
                      E('span', { title: outbound.displayName }, outbound.displayName),
                    ]),
                    E('td', { class: 'td', 'data-title': _('Type') }, outbound.type || ''),
                    E('td', { class: 'td', 'data-title': _('Latency') }, renderLatency(outbound)),
                    E(
                      'td',
                      {
            ...
            : [
                E('tr', { class: 'tr placeholder' }, [
                  E('td', { class: 'td', colspan: 4 }, E('em', {}, _('There are no subscriptions'))),
                ]),
              ]),
        ]);
    }
    ...
    function openSubscriptionModal() {
      ...
      async function refreshLatency(event?: MouseEvent) {
        ...
        if (refreshButton) {
          ...
          refreshButton.replaceChildren(renderLoaderCircleIcon24(), ' ', _('Refresh latency'));
        }
        ...
        try {
          ...
          console.log('[KROT] refreshLatency: groupResult=', groupResult ? { code: groupResult.code, outbounds: groupResult.outbounds?.length, latencies: groupResult.outbounds?.map(o => o.latency) } : null);
          if (groupResult) {
          ...
          const CONCURRENCY = 4;
          let cursor = 0;
          const tasks = outboundsToTest.map(
            (outbound) => async () => {
              const result = await onTestOutboundLatency(outbound.code, {
                force: true,
              });
              console.log('[KROT] refreshLatency: outboundResult', outbound.code, result ? { latencies: result.outbounds?.map(o => ({ code: o.code, latency: o.latency })) } : null);
              if (result) {
          ...
          showToast(
            _('Failed to refresh latency') + ': ' + (err instanceof Error ? err.message : String(err)),
            'error',
          );
        } finally {
          latencyRefreshRunning = false;
      ...
      ui.showModal(
        _('Subscription') + `: ${subscriptionTitle}`,
        [
          E(
            'div',
            { class: 'pdk_dashboard-page__subscription-modal' },
            table,
          ),
          E('div', { class: 'right' }, [
            refreshButton,
    ...
    const subscriptionFactText = [
      subscriptionTrafficText ? `${_('Traffic')}: ${subscriptionTrafficText}` : '',
      subscriptionExpireText ? `${_('Expires')}: ${subscriptionExpireText}` : '',
    ]
      .filter(Boolean)
      .join(' · ');
    const subscriptionInfo = firstMetadata
      ? [
          ...
          ...(subscriptionFactText ? [E('br'), E('span', {}, subscriptionFactText)] : []),
        ]
      : subscriptionTitle;
    ...
    return E(
      ...
      [
        E('h3', {}, _('Subscription')),
        E('table', { class: 'table pdk_dashboard-page__subscription-summary-table' }, [
          E('tr', { class: 'tr table-titles' }, [
            E('th', { class: 'th' }, _('Name')),
          ...
          E('tr', { class: 'tr cbi-rowstyle-1' }, [
                ...
                ? [
                    ...
                    E('small', {}, [
                      ...(renderCountryFlag(selectedOutbound.country)
                        ? [renderCountryFlag(selectedOutbound.country)!, ' ']
                        : []),
                      E('span', { title: selectedOutbound.displayName }, selectedOutbound.displayName),
                    ]),
                  ]
                : []),
            ]),
            E('td', { class: 'td', 'data-title': _('Subscription info') }, subscriptionInfo),
            E(
              'td',
            ...
            E(
              ...
              E('div', { class: 'right pdk_dashboard-page__subscription-actions' }, [
                E(
                  'button',
                  {
