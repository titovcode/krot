...
function renderFailedState(title: string) {
  return E(
    ...
    [
      E(
        'div',
        { class: 'ifacebox-head center active pdk_dashboard-page__widgets-section__item__title' },
...
function renderDefaultState({ title, items }: IRenderWidgetProps) {
  return E('div', { class: 'ifacebox pdk_dashboard-page__widgets-section__item' }, [
    E(
      'div',
      { class: 'ifacebox-head center active pdk_dashboard-page__widgets-section__item__title' },
      E('strong', {}, title),
...
export function renderWidget(props: IRenderWidgetProps) {
  if (props.loading) {
    return props.items.length ? renderDefaultState(props) : renderLoadingState(props.title);
  }
