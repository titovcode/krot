...
async function fetchDashboardSectionsOnce(mountId: number) {
  ...
  try {
    ...
    const nextData =
      data.length || !current.data.length ? data : current.data;
...
async function handleTestGroupLatency(
  ...
  if (
    ...
    return store
      .get()
      .sectionsWidget.data.find((section) => section.sectionName === sectionName);
  }
...
async function refreshDashboardLatencies() {
  ...
  const sectionTargets = sections.map(getDashboardLatencyTarget).filter(Boolean) as {
    sectionName: string;
    tag: string;
    group: boolean;
  }[];
...
async function renderSectionsWidget() {
  ...
  return preserveScrollForPage(() => {
    container.replaceChildren(...renderedWidgets, ...renderedSubscriptionDetails);
  });
}
