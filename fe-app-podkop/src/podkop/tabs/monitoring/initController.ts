...
function buildRouteDisplayNames(sections: Podkop.ConfigSection[]) {
  ...
  sections
    ...
    .forEach((section) => {
      ...
      map[sectionName] = displayName;
      map[sectionName.toLowerCase()] = displayName;
      ...
      if (section.action === 'direct') {
        map.direct = displayName;
        map['direct-out'] = displayName;
      }
    });
    ...
    (next: StoreType, prev: StoreType, diff: Partial<StoreType>) => {
      if (
        ...
        const isMonitoringVisible =
          next.tabService.current === 'monitoring' ||
          next.tabService.current === 'dashboard';
