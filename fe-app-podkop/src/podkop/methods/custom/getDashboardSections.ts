...
const DASHBOARD_SECTION_CACHE_DIR = '/var/run/krot/section-cache';
...
function getSectionAction(section: Podkop.ConfigSection) {
  return (section.action || '').toLowerCase();
}

function getSectionIcon(section: Podkop.ConfigSection) {
  return section.icon || 'port_up';
}
...
export async function getDashboardSections(
  ...
  const proxies = Object.entries(clashProxies.success ? clashProxies.data?.proxies || {} : {}).map(
    ([key, value]) => ({
      code: key,
      value,
    }),
  ...
  const data = await Promise.all(
    configSections
      .filter(
        (section) =>
          ['direct', 'proxy', 'outbound', 'vpn', 'byedpi'].includes(
      ...
      .map(async (section) => {
        ...
        if (sectionAction === 'direct') {
          const outbound = proxies.find((proxy) => proxy.code === 'direct-out');

          return {
            withTagSelect: false,
            code: outbound?.code || 'direct-out',
            sectionName,
            displayName,
            icon,
            enabled,
            ...
            outbounds: [
              {
                ...
                type: outbound?.value?.type || 'Direct',
                selected: true,
                canCopyLink: false,
              },
            ],
