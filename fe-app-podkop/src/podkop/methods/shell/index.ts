...
const COMPONENT_ACTION_STATE_DIR = '/var/run/krot/component-actions';
...
export const PodkopShellMethods = {
  ...
  getClashApiProxyLatency: async (
    tag: string,
    options: { timeout?: string; quick?: boolean } = {},
  ) =>
  ...
  componentAction: async (
    ...
    const args = [Podkop.AvailableMethods.COMPONENT_ACTION_ASYNC, component, action];
    if (actionArg) {
      args.push(actionArg);
    }
            ...
            if (
              ...
              return {
                ...
                data: {
                  ...
                  component,
                  action,
                  message: _('K.R.O.T. has been installed'),
  ...
  exportConfig: () =>
    callBaseMethod<string>(Podkop.AvailableMethods.BACKUP_CONFIG),

  importConfig: async (
    config: string,
  ): Promise<Podkop.MethodResponse<string>> => {
    await (
      fs as { write: (path: string, data: string) => Promise<void> }
    ).write('/tmp/krot_config_import.txt', config);

    return callBaseMethod<string>(Podkop.AvailableMethods.BACKUP_CONFIG, [
      '/tmp/krot_config_import.txt',
    ]);
  },

  hubListSources: async (): Promise<Podkop.MethodResponse<string>> => {
    return callBaseMethod<string>(Podkop.AvailableMethods.HUB_LIST_SOURCES);
  },

  hubGetModules: async (): Promise<Podkop.MethodResponse<string>> => {
    return callBaseMethod<string>(
      Podkop.AvailableMethods.HUB_GET_MODULES,
      [],
      '/usr/bin/krot',
      { timeout: 30000 },
    );
  ...
  hubAddSource: async (
    ...
    return callBaseMethod<string>(
      ...
      ['hub', 'add_source', sourceArg],
    );
  },

  hubRemoveSource: async (
    moduleId: string,
  ): Promise<Podkop.MethodResponse<string>> => {
    return callBaseMethod<string>(
      Podkop.AvailableMethods.COMPONENT_ACTION_ASYNC,
      ['hub', 'remove_source', moduleId],
    );
  },
};
