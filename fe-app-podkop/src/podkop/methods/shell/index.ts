...
export const PodkopShellMethods = {
  ...
  importConfig: async (config: string): Promise<Podkop.MethodResponse<string>> => {
    await (fs as { write: (path: string, data: string) => Promise<void> }).write(
      '/tmp/krot_config_import.txt',
      config,
    );

    return callBaseMethod<string>(
      Podkop.AvailableMethods.BACKUP_CONFIG,
      ['/tmp/krot_config_import.txt'],
    );
  },
};
